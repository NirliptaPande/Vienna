...
stack_df = stack_df[np.logical_and(stack_df['start_datetime'] >= np.datetime64('2019-01-01'), stack_df['start_datetime'] < np.datetime64('2019-01-05'))]
#now stack_df has paths of both S1 and S2 b/w Jan beginning to end of April
stack_df = stack_df[source_artefact_name = 'VVVH'] #sorted out the S1 data
...
for start_datetime_np in sorted_datetimes:
    geotiff_path = Path(start_df.loc[stack_df['start_datetime'] == start_datetime_np][raster_path])
    
    partial_raster = Raster(geotiff_path, window=window)
    ip.append(partial_raster)
ip = np.array(ip) 
# df is the  final dataset, 1D in this case, with 2 columns, time and Y values
out = np.zeros([80,50])
for i in range(80):
    for j in range(50):
        df = pd.DataFrame(ip[:,i,j])
        m = Prophet()
        m.fit(df)
        future = m.make_future_dataframe(periods=1)
        forecast = m.predict(future)
        out[i][j]=forecast['yhat']
